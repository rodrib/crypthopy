...
def main():
    ...
    if seleccion == "Ofuscador de Texto":
        ...
        if carga_local:
            imagen_archivo = st.file_uploader("Seleccione una imagen:", type=["jpg", "jpeg", "png"])
            if imagen_archivo is not None:
                imagen_original = Image.open(imagen_archivo)
        else:
        ...
        if st.button("Ofuscar Imagen") and imagen_original:
            # Ofuscar la imagen
            imagen_ofuscada = ofuscar_imagen(imagen_original)
            # Mostrar la imagen original y ofuscada
            st.image([imagen_original, imagen_ofuscada], caption=["Imagen Original", "Imagen Ofuscada"], width=300)
